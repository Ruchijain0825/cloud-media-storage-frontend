...
import { fetchRootFolders, fetchFolderChildren, updateFolder, moveFolder, deleteFolder, fetchMoveFolders } from "./apis/folder.api";
...
export default function FileExplorer({ currentFolderId = null, setCurrentFolderId, searchQuery = "", sortBy = "name", viewMode = "list" }) {
    ...
    if (rootQuery.isLoading || childrenQuery.isLoading || searchQueryResult.isLoading) {
        return (
    ...
    if (rootQuery.isError || childrenQuery.isError || searchQueryResult.isError) {
        const error = rootQuery.error || childrenQuery.error || searchQueryResult.error;
    ...
    const handleUpdateFolder = async (folderId) => {
        ...
        const previousChildrenData = queryClient.getQueryData(["folder-children", currentFolderId]);
        ...
        try {
            ...
            queryClient.setQueryData(["folder-children", currentFolderId], previousChildrenData);
    ...
    const handleMoveDestination = async (folder) => {
        if (!movingFolder || folder.id === movingFolder.id) {
            return;
        }
        ...
        try {
            ...
            setMoveFolders(children.filter((item) => item.id !== movingFolder.id));
    ...
    const handleMoveBack = async () => {
        ...
        const parentId = previousPath.length > 0
    ...
    const handleMoveFolder = async () => {
        if (!movingFolder || !moveDestinationId || isMoving) {
            return;
        }
        ...
        const previousCurrentChildrenData = queryClient.getQueryData(["folder-children", currentFolderId]);

        const previousDestinationData = queryClient.getQueryData(["folder-children", destinationId]);
        ...
        queryClient.setQueryData(["root-folders"], (oldData) => {
            ...
            return {
                ...oldData,
                folders: oldData.folders?.filter((folder) => folder.id !== sourceFolderId)
            };
        });
        ...
        queryClient.setQueryData(["folder-children", currentFolderId], (oldData) => {
            ...
            return {
                ...
                children: {
                    ...oldData.children,
                    folders: oldData.children.folders?.filter((folder) => folder.id !== sourceFolderId)
                }
            };
        });
        ...
        queryClient.setQueryData(["folder-children", destinationId], (oldData) => {
            ...
            const alreadyExists = oldData.children.folders?.some((folder) => folder.id === sourceFolderId);
        ...
        try {
            ...
            queryClient.setQueryData(["folder-children", currentFolderId], previousCurrentChildrenData);

            queryClient.setQueryData(["folder-children", destinationId], previousDestinationData);
    ...
    return (
        <>
            <div className="bg-white rounded-2xl overflow-visible">
                ...
                {!isSearching && (
                    <Breadcrumb currentFolderId={currentFolderId} folderPath={folderPath} onRoot={handleGoToRoot} onBreadcrumb={handleBreadcrumbClick} />
                )}
                ...
                {isSearching && searchQueryResult.data?.pagination && (
                    <div className="flex items-center justify-center gap-4 py-4 border-t border-slate-100">

                        <button type="button" onClick={() => setSearchPage((page) => page - 1)} disabled={searchPage === 1} className="px-4 py-2 border border-slate-200 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50">
                            Previous
                        </button>
                        ...
                        <button type="button" onClick={() => setSearchPage((page) => page + 1)} disabled={!searchQueryResult.data.pagination.hasNextPage} className="px-4 py-2 border border-slate-200 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50">
                            Next
                        </button>
            ...
            <PreviewModal selectedFile={selectedFile} onClose={() => setSelectedFile(null)} />
