...
const fetchStarredItems = async () => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stars`, {
    headers: {
...
const fetchTrashItems = async () => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/trash`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export default function Dashboard() {
  ...
  const handleUnstar = async (star) => {
    const resourceType = star.resource_type;

    const resourceId = star.resource_id;
    ...
    try {
      ...
      await toggleStar(resourceType, resourceId, true);

      queryClient.setQueryData(["starred-items"], (oldData) => {
        if (!oldData) {
          return oldData;
        }
      ...
      alert(error.message || "Failed to remove star");
    } finally {
  ...
  const handleStarredPreview = (star) => {
    ...
    const fileUrl =
      star.url || star.file_url || star.public_url || star.storage_url;

    if (!fileUrl) {
      alert("Preview URL is not available for this file.");
      return;
    }
    ...
    setSelectedFile({
      ...star,
      id: star.resource_id,
      name: star.name || star.resource_id,
      url: fileUrl,
    });
  };
  ...
  return (
    <>
            ...
            <div className="p-4 sm:p-6 lg:p-8 min-w-0">
              ...
              {starredQuery.isLoading && (
                <div className="bg-white border border-slate-200 rounded-2xl min-h-[300px] flex items-center justify-center">
                  <p className="text-slate-500">Loading starred items...</p>
                </div>
              )}
                    ...
                    <div className="overflow-x-auto">
                      <div className="min-w-[520px]">
                        ...
                        {starredQuery.data.stars.map((star) => {
                          const key = `${star.resource_type}-${star.resource_id}`;

                          const isRemoving = starLoadingId === key;
                          ...
                          return (
                            ...
                            >
                              <button
                                type="button"
                                onClick={() => handleStarredPreview(star)}
                                disabled={star.resource_type !== "file"}
              ...
              >
                <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-slate-200">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-1">File Preview</p>
                  </div>
          ...
          {activeSection === "recent" && (
            <div className="p-4 sm:p-6 lg:p-8 min-w-0">
              ...
              {recentQuery.isLoading && (
                <div className="bg-white border border-slate-200 rounded-2xl min-h-[300px] flex items-center justify-center">
                  <p className="text-slate-500">Loading recent files...</p>
                </div>
              )}
                  ...
                  {recentQuery.data?.files?.length > 0 ? (
                    <div className="overflow-x-auto">
                      <div className="min-w-[560px]">
                        ...
                        {recentQuery.data.files.map((file) => (
                          <button
                            key={file.id}
                            type="button"
      ...
      <PreviewModal selectedFile={selectedFile} onClose={closePreview} />
